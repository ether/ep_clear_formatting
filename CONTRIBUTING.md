# Contributing to ep_clear_formatting

Thanks for helping improve `ep_clear_formatting` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_clear_formatting` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_clear_formatting from your local clone
pnpm run plugins i --path ../ep_clear_formatting

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_clear_formatting/` are picked up after a server restart.
...
* Etherpad version, ep_clear_formatting version, browser + OS
* Steps to reproduce
